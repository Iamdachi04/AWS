...
import * as state from "./state"; // Assuming this module provides state management/access using the DB
...
app.post("/api/create-answer", (req: Request, res: Response) => {
  let receivedData: string | undefined = req.body.data;

  if (!receivedData || typeof receivedData !== 'string' || receivedData.trim() === "") {
    console.error("Invalid or empty 'data' received for /api/create-answer:", req.body);
    return res.status(400).json({ error: "Invalid or empty 'data' string provided in the JSON body as required by the assignment" });
  }

  console.log(`Assignment state updated successfully by POST: "${receivedData}"`);
  qvevri = receivedData; // Store the received data

  res.status(200).json({ result: "Message received and stored successfully for assignment." });
});

// Assignment Endpoint: Returns the currently stored text from the assignmentState variable.
// Accessible at GET /api/get-answer
app.get("/api/get-answer", (req: Request, res: Response) => {
    console.log("Frontend requesting simple assignment data from /api/get-answer...");
     if (!qvevri || qvevri.trim() === "") {
        res.status(200).json({ result: "" });
    } else {
        res.status(200).json({ result: qvevri });
    }
});


// --- Start Server ---
app.listen(PORT as number, "0.0.0.0", () => { // MODIFIED: it listens to any type of port you give to it
  console.log(`Backend server running at http://0.0.0.0:${PORT}`); // MODIFIED: Updated log to reflect 0.0.0.0
  console.log(`Initial Current Day: 0`); // Keeping existing log
});